...
## P-271
sensor_distance = 2.305  # m
no_flow_diff = -1.7702570998033966  # C
...
#]
csv_files = [
    'data/xenonnt-lxe_purification-p271_heat-02_01_2020_11_00_00-02_01_2020_18_00_00.csv',
    'data/xenonnt-lxe_purification-p271_heat-01_31_2020_08_00_00-01_31_2020_18_00_00.csv',   
]
...
te272_vals_array = sensors_dataframe['PUR_TE272_TPUMPOUTLET.PI'].values.astype(np.float64)
te288_vals_array = sensors_dataframe['PUR_TE288_THEOUTLET.PI'].values.astype(np.float64)
...
for pump_frequency in pump_frequencies:
    ...
    for time_interval in time_intervals[pump_frequency]:
        ...
        ti_vals.append(np.mean(te288_vals_array[interval_array]))
        tf_vals.append(np.mean(te272_vals_array[interval_array]))
        flow_vals.append(flow)
        deltap_vals.append(deltap)
        inletp_vals.append(pt275_val - deltap)
    flows[pump_frequency] = flow_vals
    deltaps[pump_frequency] = deltap_vals
    inletps[pump_frequency] = inletp_vals

    tis[pump_frequency] = ti_vals
    tfs[pump_frequency] = tf_vals
...
eff_simple = lambda x, a, b: a*x + b
xss = []
yss = []
for j, freq in enumerate(freqs):
    tdiffs = np.array(tfs[freq]) - np.array(tis[freq]) - no_flow_diff - (sensor_distance/(2.5354*np.array(flows[freq])))
    xdata = deltaps[freq]
    dh = np.array([
        cp.PropsSI(
            'H', 'T', tis[freq][i] + 273.15 + tdiffs[i], 'P', (inletps[freq][i]+deltaps[freq][i]) * pa_per_bar, 'Xenon'
        ) - cp.PropsSI(
            'H', 'T', tis[freq][i] + 273.15, 'P', inletps[freq][i] * pa_per_bar, 'Xenon'
        ) for i in range(len(tis[freq]))
    ])
    rho = np.array([cp.PropsSI(
            'D', 'T', tis[freq][i] + 273.15 + tdiffs[i], 'P', (inletps[freq][i]+deltaps[freq][i]) * pa_per_bar, 'Xenon'
        ) for i in range(len(tis[freq]))
    ])
    #print(rho)
    print(pa_per_bar*np.mean(deltaps[freq]), np.mean(dh), np.mean(rho))
    xs = np.asarray(flows[freq])
    ys = np.asarray(deltaps[freq]/dh/rho)*pa_per_bar
    xss.extend(xs.tolist())
    yss.extend(ys.tolist())
# save the data
pd.DataFrame({'flow': xss, 'eta': yss}).to_csv('data/p271_eta.csv')


### P-272
sensor_distance = 2.73  # m
no_flow_diff = -0.45  # degC
...
#]
csv_files = [
    'data/xenonnt-lxe_purification-p272_heat-02_05_2020_05_00_00-02_09_2020_10_00_00.csv',
    'data/xenonnt-lxe_purification-p272_heat-12_06_2021_08_00_00-12_31_2021_20_00_00.csv',   
]
...
te273_vals_array = sensors_dataframe['PUR_TE273_TPUMPOUTLET.PI'].values.astype(np.float64)
te288_vals_array = sensors_dataframe['PUR_TE288_THEOUTLET.PI'].values.astype(np.float64)
...
for pump_frequency in pump_frequencies:
    ...
    for time_interval in time_intervals[pump_frequency]:
        ...
        ti_vals.append(np.mean(te288_vals_array[interval_array]))
        tf_vals.append(np.mean(te273_vals_array[interval_array]))
        flow_vals.append(flow)
        deltap_vals.append(deltap)
        inletp_vals.append(pt276_val - deltap)
    flows[pump_frequency] = flow_vals
    deltaps[pump_frequency] = deltap_vals
    inletps[pump_frequency] = inletp_vals
...
for j, freq in enumerate(freqs):
    ...
    yss.extend(ys.tolist())
# save the data
pd.DataFrame({'flow': xss, 'eta': yss}).to_csv('data/p272_eta.csv')


## etas combined
df = pd.read_csv('data/p271_eta.csv')

plt.plot(df.flow, df.eta, '.', label='P-271')
df = pd.read_csv('data/p272_eta.csv')

plt.plot(df.flow, df.eta, '.', label='P-272')
plt.grid(alpha=0.3)
plt.xlabel('LXe Pump Flow [liter/min]')
plt.ylabel(r'Pump Efficiency $\eta$')
plt.legend()
if True:
    plt.savefig('plots/combined_eta.pdf')
plt.show()
